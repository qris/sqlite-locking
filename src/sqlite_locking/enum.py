...
from sqlite_locking import python_module
from sqlite_locking.python_module import sqlite3_db_config, sqlite3_db_status, sqlite3_txn_state
...
class SqliteDatabaseStatus(Enum):
    ...
    def get(self, db: sqlite3.Connection) -> int:
        ...
        if result[0] != 0:
            raise ValueError(f"sqlite3_db_status({self.name}) failed with rc={result[0]}")
        return result[1]


class SqliteDBConfig(IntEnum):
    """
    Possible values of the op argument to sqlite3_setconfig.

    https://docs.python.org/3/library/sqlite3.html#sqlite3.Connection.setconfig
    https://docs.python.org/3/library/sqlite3.html#sqlite3.SQLITE_DBCONFIG_DEFENSIVE
    https://sqlite.org/c3ref/c_dbconfig_defensive.html#sqlitedbconfiglookaside
    """

    ENABLE_FKEY = python_module.SQLITE_DBCONFIG_ENABLE_FKEY
    ENABLE_TRIGGER = python_module.SQLITE_DBCONFIG_ENABLE_TRIGGER
    ENABLE_FTS3_TOKENIZER = python_module.SQLITE_DBCONFIG_ENABLE_FTS3_TOKENIZER
    ENABLE_LOAD_EXTENSION = python_module.SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION
    NO_CKPT_ON_CLOSE = python_module.SQLITE_DBCONFIG_NO_CKPT_ON_CLOSE
    ENABLE_QPSG = python_module.SQLITE_DBCONFIG_ENABLE_QPSG
    TRIGGER_EQP = python_module.SQLITE_DBCONFIG_TRIGGER_EQP
    RESET_DATABASE = python_module.SQLITE_DBCONFIG_RESET_DATABASE
    DEFENSIVE = python_module.SQLITE_DBCONFIG_DEFENSIVE
    WRITABLE_SCHEMA = python_module.SQLITE_DBCONFIG_WRITABLE_SCHEMA
    DQS_DDL = python_module.SQLITE_DBCONFIG_DQS_DDL
    DQS_DML = python_module.SQLITE_DBCONFIG_DQS_DML
    LEGACY_ALTER_TABLE = python_module.SQLITE_DBCONFIG_LEGACY_ALTER_TABLE
    ENABLE_VIEW = python_module.SQLITE_DBCONFIG_ENABLE_VIEW
    LEGACY_FILE_FORMAT = python_module.SQLITE_DBCONFIG_LEGACY_FILE_FORMAT
    TRUSTED_SCHEMA = python_module.SQLITE_DBCONFIG_TRUSTED_SCHEMA
